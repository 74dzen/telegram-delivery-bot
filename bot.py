...
DPD_ACCOUNTS = [
    {
        "clientNumber": os.getenv("DPD_CLIENT_NUMBER_1"),
        "clientKey": os.getenv("DPD_CLIENT_KEY_1"),
    },
    {
        "clientNumber": os.getenv("DPD_CLIENT_NUMBER_2"),
        "clientKey": os.getenv("DPD_CLIENT_KEY_2"),
    },
]
...
DPD_FORCE_COURIER_PICKUP = {"BZP"}  # 18:00 требует selfPickup=False для «как на сайте»
...
def calculate_cdek_delivery(city_from, city_to, dims):
    ...
    response = requests.post(CDEK_TARIFFLIST_URL, headers=headers, json=payload, verify=certifi.where(), timeout=25)
    if response.status_code != 200:
    ...
    return "\n".join(
        [f"📦 {k}: {v['price']} руб., срок {v['term']}" if v else f"📦 {k}: тариф недоступен" for k, v in categories.items()]
    )
...
def _build_dpd_client() -> Client:
    """
    Строим zeep-клиент. Без нестандартных аргументов — совместимо с текущей версией zeep.
    """
    return Client(DPD_WSDL_URL)
...
async def _prompt_city_selection_single(update: Update, which: str, candidates):
    ...
    header = (
        f"Найдено несколько вариантов для "
    ...
    if total > len(candidates):
        header += f"\nПоказано {len(candidates)} из {total}. Можно уточнить область: «Октябрьский, Башкортостан»."
...
async def _prompt_city_selection_both(update: Update, cands_from, cands_to):
    ...
    header = [
        "Найдено несколько вариантов для обоих городов.",
    ...
    blocks = []
    blocks.append("\n".join(header))
    blocks.append("\n".join(["", "Город отправки:"] + [_format_candidate_line(i, c, n, t, s) for i, (c, n, t, s) in enumerate(cands_from, start=1)]))
    blocks.append("\n".join(["", "Город доставки:"] + [_format_candidate_line(i, c, n, t, s) for i, (c, n, t, s) in enumerate(cands_to, start=1)]))
...
async def _resolve_cities_or_ask(update: Update, context: CallbackContext, service: str, text: str):
    ...
    if tag == "error":
        await update.message.reply_text(status[1])
        return None
    if tag == "ask_one":
    ...
    if tag == "ask_both":
        ...
        context.user_data["city_select_both"] = {"from": cands_from, "to": cands_to, "rest": rest, "service": service}
...
async def _do_calc_and_reply_dpd(update: Update, code_from: str, code_to: str, rest_tokens):
    """Быстрый ответ пользователю и расчёт в фоне."""
    try:
...
async def handle_input(update: Update, context: CallbackContext):
    ...
    logger.info("update in, text='%s'", text)
    ...
    if "city_select_both" in context.user_data:
        ...
        if sel_from[0] == "index":
            ...
            if not code_exists(code_from):
                await update.message.reply_text("Неверный код для города отправки.")
                return
        if sel_to[0] == "index":
            ...
            if not code_exists(code_to):
                await update.message.reply_text("Неверный код для города доставки.")
                return
        ...
        if service == "DPD":
            ...
            name_from = name_by_code(code_from)
            name_to = name_by_code(code_to)
            if not name_from or not name_to:
                await update.message.reply_text("Коды городов выбраны, но не удалось восстановить названия для CDEK.")
                return
            dims = parse_dims_tokens(rest)
            if dims is None:
                await update.message.reply_text("Ошибка: не распознаны габариты. Укажите 4 числа или шаблон.")
                return
            result = calculate_cdek_delivery(name_from, name_to, dims)
    ...
    if "city_select_single" in context.user_data:
        ...
        if not sel:
            await update.message.reply_text("Введите «1)» (индекс) или код города.")
            return
        if sel[0] == "index":
            ...
            if not code_exists(chosen_code):
                await update.message.reply_text("Неверный код города.")
                return

        which = st["which"]
        other_code = st["other_code"]
        rest = st["rest"]
        service = st["service"]
        if which == "from":
            code_from = chosen_code
            code_to = other_code
        else:
            code_to = chosen_code
            code_from = other_code
    ...
    if service == "СДЭК":
        ...
        if dims is None:
            ...
            return
        city_from_name = name_by_code(code_from)
        city_to_name = name_by_code(code_to)
        if not city_from_name or not city_to_name:
            ...
            return
        result = calculate_cdek_delivery(city_from_name, city_to_name, dims)
        await send_long_message(update, "Результат расчета:\n" + result)
    else:
        await _do_calc_and_reply_dpd(update, code_from, code_to, rest)
...
if __name__ == "__main__":
    ...
    base_url = os.getenv("WEBHOOK_BASE_URL")  # например: https://your-service.onrender.com
    if not base_url:
        raise RuntimeError("Не задан WEBHOOK_BASE_URL в переменных окружения Render")
    path = os.getenv("WEBHOOK_PATH", "webhook").lstrip("/")  # можно задать свой секретный путь

    full_webhook_url = f"{base_url.rstrip('/')}/{path}"

    async def _post_init(app: Application):
        # Ставим вебхук ПОСЛЕ старта HTTP-сервера, с ретраями
        for attempt in range(1, 6):
            try:
                await app.bot.set_webhook(full_webhook_url)
                logger.info("Webhook set OK: %s", full_webhook_url)
                break
            except Exception as e:
                logger.warning("set_webhook failed (try %s/5): %s", attempt, e)
    ...
    # Запускаем сервер сразу (без мгновенной установки вебхука), чтобы Render увидел открытый порт
    application.post_init = _post_init  # PTB v22+
    application.run_webhook(
        listen="0.0.0.0",
        port=port,
        url_path=path,     # сам маршрут нашего хука
        webhook_url=None,  # вебхук ставим позже в _post_init
    )
