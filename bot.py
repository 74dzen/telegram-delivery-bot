...
DPD_FORCE_COURIER_PICKUP = {"BZP"}  # для «как на сайте»
...
def get_cdek_token():
    response = requests.post(
        CDEK_AUTH_URL,
        data={"grant_type": "client_credentials", "client_id": CDEK_CLIENT_ID, "client_secret": CDEK_CLIENT_SECRET},
        timeout=25,
    )
...
def calculate_cdek_delivery(city_from, city_to, dims):
    ...
    if isinstance(dims, list):
        for l, w, h, weight in dims:
            packages.append({"weight": int(float(weight) * 1000), "length": int(round(float(l))),
                             "width": int(round(float(w))), "height": int(round(float(h)))})
    else:
        l, w, h, weight = dims
        packages.append({"weight": int(float(weight) * 1000), "length": int(round(float(l))),
                         "width": int(round(float(w))), "height": int(round(float(h)))})
...
def _build_dpd_client() -> Client:
    """Без Transport(client=...) — совместимо с текущим zeep."""
...
def calculate_dpd_delivery_by_codes(code_from: str, code_to: str, rest_tokens):
    ...
    for account in DPD_ACCOUNTS:
        ...
        for length, width, height, weight in dims_list:
            ...
            for sc in list(per_user.keys()):
                if sc in DPD_FORCE_COURIER_PICKUP and user_self_pickup:
                    req2 = dict(base_req); req2['selfPickup'] = False
                    try:
        ...
        for sc in sorted(per_user.keys(), key=lambda k: per_user[k]['cost']):
            u = per_user[sc]; v = per_site.get(sc)
...
async def handle_input(update: Update, context: CallbackContext):
    ...
    if 'city_select_both' in context.user_data:
        ...
        if not sel_from or not sel_to:
            await update.message.reply_text("Введите два значения: «1) 2)» (индексы) или «код1 код2»."); return
        if sel_from[0] == 'index':
